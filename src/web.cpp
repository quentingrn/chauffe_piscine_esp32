...
function update(){var mode=q('input[name=mode]:checked').value;var manual=q('input[name="manuelEtat"]:checked');var on=manual&&manual.value==='ON';q('#autoFields').style.display=mode==='AUTO'?'block':'none';q('#manualFields').style.display=mode==='MANUEL'?'block':'none';q('#scheduleOptions').style.display=(mode==='AUTO'||on)?'block':'none';toggleSchedule();}
function toggleSchedule(){q('#scheduleTimes').style.display=q('#scheduleActive').checked?'block':'none';}
document.querySelectorAll('input[name=mode]').forEach(e=>e.addEventListener('change',update));
document.querySelectorAll('input[name=manuelEtat]').forEach(e=>e.addEventListener('change',update));
q('#scheduleActive').addEventListener('change',toggleSchedule);
update();
let history=[],heating=[],chart;
async function load(){const h=await fetch('/history.csv');const ht=await h.text();const c=await fetch('/chauffage.csv');const ct=await c.text();history=ht.trim().split('\n').slice(1).map(l=>{const p=l.split(',');return{t:new Date(p[0]),w:+p[1],a:+p[2]}});heating=ct.trim().split('\n').slice(1).map(l=>{const p=l.split(',');return{t:new Date(p[0]),on:p[1].trim()==='ON'}});const ctx=q('#chart').getContext('2d');chart=new Chart(ctx,{type:'line',data:{datasets:[{label:'Eau',borderColor:'blue',data:[],fill:false},{label:'Air',borderColor:'gray',data:[],fill:false},{label:'Chauffage',borderColor:'red',backgroundColor:'rgba(255,0,0,0.3)',data:[],stepped:true,fill:true,yAxisID:'y2'}]},options:{responsive:true,scales:{x:{type:'time'},y:{title:{display:true,text:'°C'}},y2:{display:false,min:0,max:1}}}});draw('24h');}
function draw(r){const now=new Date();const from=new Date(now-(r==='24h'?86400000:604800000));const h=history.filter(e=>e.t>=from);const s=heating.filter(e=>e.t>=from);chart.data.datasets[0].data=h.map(e=>({x:e.t,y:e.w}));chart.data.datasets[1].data=h.map(e=>({x:e.t,y:e.a}));let d=[],state=0;d.push({x:from,y:0});for(const p of s){d.push({x:p.t,y:state});state=p.on?1:0;d.push({x:p.t,y:state});}d.push({x:now,y:state});chart.data.datasets[2].data=d;chart.update();}
q('#v24').onclick=()=>draw('24h');
q('#v7').onclick=()=>draw('7d');
load();
</script></body></html>
)rawliteral";
